(function( $ ) {
    $( window ).load( function() {
            ...
            return [
                {
                    ...
                    'options' :[
                        ...
                        {'name': 'City 🔒','attributes' : {'value':'city_in_pro'} },
                        {'name': 'State 🔒','attributes' : {'value':'state_in_pro'} },
                        {'name': 'Postcode 🔒','attributes' : {'value':'postcode_in_pro'} },
                        {'name': 'Zone 🔒','attributes' : {'value':'zone_in_pro'} },
                    ]
                },
                {
                    'type': 'optgroup',
                ...
                },
                {
                    ...
                    'options': [
                        ...
                        {'name' : 'User Role 🔒', 'attributes': {'value' : 'user_role_in_pro'}},
                        {'name' : 'User Email 🔒', 'attributes': {'value' : 'user_mail_in_pro'}},
                    ]
                },
                {
                    'type': 'optgroup',
                    ...
                    'options': [
                        {'name' : 'Last order spent  🔒', 'attributes': {'value' : 'last_spent_order_in_pro'}},
                        {'name' : 'Total order spent (all time) 🔒', 'attributes': {'value' : 'total_spent_order_in_pro'}},
                        {'name' : 'Number of orders (all time) 🔒', 'attributes': {'value' : 'spent_order_count_in_pro'}},
                        {'name' : 'User repeat product 🔒', 'attributes': {'value' : 'user_repeat_product_in_pro'}},
                    ]
                },
                {
                    'type': 'optgroup',
                    ...
                    'options': [
                        ...
                        {'name' : 'Cart Subtotal (After Discount) 🔒', 'attributes': {'value' : 'cart_totalafter_in_pro'}},
                        {'name' : 'Quantity', 'attributes': {'value' : 'quantity'}},
                        {'name' : 'Weight (kg) 🔒', 'attributes': {'value' : 'weight_in_pro'}},
                        {'name' : 'Coupon 🔒', 'attributes': {'value' : 'coupon_in_pro'}},
                        {'name' : 'Shipping Class 🔒', 'attributes': {'value' : 'shipping_class_in_pro'}}
                    ]
                },
                {
                    'type': 'optgroup',
                    ...
                    'options': [
                        {'name' : 'Payment Gateway 🔒', 'attributes': {'value' : 'payment_in_pro'}},
                    ]
                },
                {
                    'type': 'optgroup',
                    ...
                    'options': [
                        {'name' : 'Shipping Method 🔒', 'attributes' : {'value' : 'shipping_method_in_pro'}},
                        {'name' : 'Shipping Total 🔒', 'attributes' : {'value' : 'shipping_total_in_pro'}},
                    ]
                }
            ];
        }
        ...
        $( 'body' ).on( 'change', '.product_dpad_conditions_condition', function() {
            ...
            if( selectedOption.includes('_in_pro') ){
                ...
                $('body').addClass('wdpad-modal-visible');
    ...
    jQuery( document ).ready( function( $ ) {
		...
		$(document).on( 'click', '.ds-plugin-setup-wizard-main .fs-actions button', function() {
			var wizardData = {
                'action': 'wdpad_plugin_setup_wizard_submit',
        ...
        $(document).on('click', '#dotsstoremain .wdpad-pro-label, .wdpad-upgrade-pro-to-unlock', function(){
            $('body').addClass('wdpad-modal-visible');
        });
        $(document).on('click', '.upgrade-to-pro-modal-main .modal-close-btn', function(){
            $('body').removeClass('wdpad-modal-visible');
        });
        ...
        var helpBeaconCookie = getCookie( 'wdpad-help-beacon-hide' );
        if ( ! helpBeaconCookie ) {
            if ( typeof Beacon === 'function' ) {
                ...
                Beacon('suggest', ['5e2971a92c7d3a7e9ae6a310', '5e297e9b04286364bc944bc2', '5e29824f2c7d3a7e9ae6a3e4', '5e2990f404286364bc944ce5', '5e29934504286364bc944d08']);
                ...
                $(document).on('click', '.dots-beacon-close', function(){
                    Beacon('destroy');
                    setCookie( 'wdpad-help-beacon-hide' , 'true', 24 * 60 );
                });
            }
        }

        // Hide and show advanced rules
        ...
        $( 'body' ).on( 'change', '#dpad_settings_select_dpad_type', function() {
            let selectedOption = $(this).find(':selected').val();
            if( selectedOption.includes('_in_pro') ){
                $(this).find(':selected').prop('selected', false);
    ...
    function upgradeToProFreemius( couponCode ) {
        ...
        handler = FS.Checkout.configure({
            ...
            image: 'https://www.thedotstore.com/wp-content/uploads/sites/1417/2023/09/WooCommerce-Dynamic-Pricing-and-Discount-Banner-New.png',
            coupon: couponCode,
        });
        handler.open({
            name: 'WooCommerce Dynamic Pricing and Discount Plugin',
